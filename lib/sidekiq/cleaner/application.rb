  ...
  class CleanerApplication
    ...
    post "/errors/retry" do
      jobs_to_retry = Sidekiq::DeadSet.new.each do |hash|
        if (hash['class'] == params['retry_error_class']) || (hash['error_class'] == params['retry_error_exception'])
          hash.retry
        end
      end
    ...
    post "/errors/delete" do
      jobs_to_delete = Sidekiq::DeadSet.new.each do |hash|
        if (hash['class'] == params['delete_error_class']) || (hash['error_class'] == params['delete_error_exception'])
          hash.delete
        end
      end
